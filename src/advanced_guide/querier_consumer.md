...
回想一下，`KvStorePersonQuerier`本身不是一个上下文（尽管它实现了`PersonQuerier<Context>`以查询上下文），因此它不实现其他上下文特性，如`PersonContext`。我们需要的是具体的上下文，如`AppContext`，将其`PersonQuerier`实现指定为`KvStorePersonQuerier`。我们可以通过定义`HasPersonQuerier`特质来实现这一点：
...
我们可以看到，`HasPersonQuerier` 特质有 `PersonContext` 和 `HasError` 作为它的父特质，表明具体的上下文还需要先实现这两个特质。由于 Rust 的一些怪异之处，该特质还需要 Sized 作为父特质，这个特质已经被大多数类型实现了，除了 `dyn Trait` 类型以外，因此我们可以在其他泛型参数中使用 Self。
